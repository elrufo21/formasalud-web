    ...
    <header className="bg-white border-b border-line px-6 py-4 flex items-center justify-between sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2.5 pl-1">
          <div className="w-8 h-8 rounded-full bg-navy text-goldpale flex items-center justify-center font-bold text-xs">
            {user ? `${user.name[0]}${user.last_name[0]}` : "AD"}
          </div>
          <div className="hidden sm:block text-left">
            <span className="text-xs font-bold text-navy block leading-none">
              {user ? `${user.name} ${user.last_name}` : "Admin"}
            </span>
